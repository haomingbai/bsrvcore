...
PressureSettings ResolvePressure(PressureKind kind, std::size_t cpu_count) {
  ...
  const std::size_t half_cpu = std::max<std::size_t>(1, cpu_count / 2);
  const std::size_t quarter_cpu = std::max<std::size_t>(1, cpu_count / 4);

  switch (kind) {
    case PressureKind::kLight:
      pressure.server_io_threads = 1;
      pressure.server_worker_threads = 1;
      pressure.client_concurrency = 1;
      break;
    case PressureKind::kBalanced:
      pressure.server_io_threads = quarter_cpu;
      pressure.server_worker_threads = half_cpu;
      pressure.client_concurrency =
          std::max<std::size_t>(4, pressure.server_worker_threads * 4);
      break;
    case PressureKind::kSaturated:
      pressure.server_io_threads = half_cpu;
      pressure.server_worker_threads = std::max<std::size_t>(1, cpu_count);
      pressure.client_concurrency =
          std::max<std::size_t>(16, pressure.server_worker_threads * 8);
      break;
    case PressureKind::kOverload:
      pressure.server_io_threads = half_cpu;
      pressure.server_worker_threads = std::max<std::size_t>(2, cpu_count * 2);
      pressure.client_concurrency =
          std::max<std::size_t>(32, pressure.server_worker_threads * 8);
      break;
...
RunSettings ResolveRunSettings(const CliConfig& cli) {
  ...
  if (cli.server_threads_override.has_value() ||
    ...
    if (cli.server_io_threads_override.has_value()) {
      pressure.server_io_threads = *cli.server_io_threads_override;
    }
    if (cli.server_worker_threads_override.has_value()) {
      pressure.server_worker_threads = *cli.server_worker_threads_override;
    }
    if (cli.client_concurrency_override.has_value()) {
      pressure.client_concurrency = *cli.client_concurrency_override;
    }
    if (pressure.server_io_threads == 0 ||
        pressure.server_worker_threads == 0 ||
        pressure.client_concurrency == 0) {
      throw std::invalid_argument(
          "server io/worker threads and client-concurrency must be positive");
    }
  ...
  for (PressureKind kind : pressure_kinds) {
    ...
    const auto key = std::make_tuple(pressure.server_io_threads,
