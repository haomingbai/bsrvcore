...
13. [Client tasks (HTTP/HTTPS + SSE)](client-tasks.md) — Goal: call other services (HTTP), consume SSE, build upload requests from files, use sessions with cookie jars, route through a proxy with explicit client-pipeline composition, or understand connection management.
